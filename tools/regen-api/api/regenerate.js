...
const VOICE_ID = process.env.ELEVEN_VOICE_ID || 'SzhLxXqLBlrRykTRhsSA';
const TEAM = 'Bloordale';

// Same dial positions as tools/generate_announcements.py.
...
function phrase({ firstName, lastName, pronunciation, number, withNumbers }) {
  const name = (pronunciation || `${firstName} ${lastName}`).trim();
  if (withNumbers && number != null) {
    return `Now batting for ${TEAM}: number ${number}, ${name}!`;
  }
...
export default async function handler(req, res) {
  ...
  const body = req.body || {};
  const { passcode, firstName, lastName, pronunciation, number, withNumbers } = body;
  ...
  const text = phrase({ firstName, lastName, pronunciation, number, withNumbers });
